...
let now=new Date(), Y=now.getFullYear(), M=now.getMonth(), data={}, selectedDay=null, selectedPhotos=[], selectedGPS=null;
...
function render(){document.getElementById('monthName').textContent=MONTHS[M];document.getElementById('monthYear').textContent=Y;renderStats();renderTimeline();renderCalendar();renderSettings();renderTodayNotice();}
function renderStats(){let h=0,w=0,p=0,s=0,l=0,photos=0;Object.values(data).forEach(e=>{if(e.type==='work'){w++;h+=Number(e.hrs||0)}else if(e.type==='pushim')p++;else if(e.type==='semure')s++;else if(e.type==='leje')l++;photos+=(Array.isArray(e.photos)?e.photos.length:(e.photo?1:0))});let entries=Object.keys(data).length;document.getElementById('totalHours').textContent=fmtH(h)+'h';document.getElementById('workDays').textContent=w+' ditë pune';document.getElementById('totalEntries').textContent=entries;document.getElementById('photoCount').textContent=photos+' foto';document.getElementById('todayHint').textContent=new Date().toLocaleDateString('sq-CH');document.getElementById('statEntries').textContent=entries+'/'+daysInMonth();document.getElementById('monthProgress').style.width=Math.min(100,entries/daysInMonth()*100)+'%';const rows=document.getElementById('summaryRows');rows.textContent='';[['Punë',w],['Pushim',p],['Sëmurë',s],['Leje',l],['Foto',photos]].forEach(([label,value])=>{const row=makeEl('div','row');row.appendChild(makeEl('span','',label));row.appendChild(makeEl('b','',String(value)));rows.appendChild(row)})}
function renderTimeline(){const box=document.getElementById('timeline');box.textContent='';const items=Object.entries(data).map(([d,e])=>({d:Number(d),e})).sort((a,b)=>b.d-a.d);if(!items.length){const empty=makeEl('div','empty');empty.append('Ende nuk ka regjistrime për këtë muaj.');empty.appendChild(document.createElement('br'));empty.append('Prek + për të shtuar ditën e parë.');box.appendChild(empty);return}items.forEach(({d,e})=>{const dt=new Date(Y,M,d);const photos=normalizePhotos(e.photos||e.photo||[]);const pc=photos.length;const spc=photos.filter(p=>p.selected!==false).length;const div=document.createElement('article');div.className='entry';div.onclick=()=>openDay(d);const pill=makeEl('div','datepill');const pillInner=document.createElement('div');pillInner.appendChild(makeEl('b','',String(d)));pillInner.appendChild(makeEl('span','',DOWS[dt.getDay()]));pill.appendChild(pillInner);const body=document.createElement('div');body.appendChild(makeEl('h3','',`${typeName(e.type)} ${e.type==='work'&&e.hrs?'· '+fmtH(e.hrs)+'h':''}`));body.appendChild(makeEl('p','',[e.job,e.loc,e.notes].filter(Boolean).join(' · ')||'Pa shënime'));const meta=makeEl('div','meta');if(e.job)meta.appendChild(makeEl('span','chip','# '+e.job));if(e.loc)meta.appendChild(makeEl('span','chip','📍 '+e.loc));if(e.gps)meta.appendChild(makeEl('span','chip','GPS ✓'));if(pc)meta.appendChild(makeEl('span','chip',`📷 ${spc}/${pc}`));body.appendChild(meta);div.append(pill,body);box.appendChild(div)})}
function renderCalendar(){const g=document.getElementById('calGrid');g.textContent='';for(let i=0;i<offset();i++){const e=document.createElement('div');e.className='day emptyday';g.appendChild(e)}for(let d=1;d<=daysInMonth();d++){const e=data[d];const cell=document.createElement('button');cell.className='day '+(e?'has ':'')+(d===now.getDate()&&M===now.getMonth()&&Y===now.getFullYear()?'today':'');cell.onclick=()=>openDay(d);cell.appendChild(makeEl('b','',String(d)));if(e){const dot=makeEl('i','dot '+(/^(work|pushim|semure|leje)$/.test(e.type)?e.type:'work'));cell.appendChild(dot);if(e.type==='work')cell.appendChild(makeEl('span','hrs',fmtH(e.hrs)+'h'))}g.appendChild(cell)}}
...
function renderPhotos(){const g=document.getElementById('photosGrid');g.textContent='';selectedPhotos.forEach((p,i)=>{const d=document.createElement('div');d.className='photo '+(p.selected===false?'off':'');const img=document.createElement('img');img.src=p.src;img.alt=p.name||'foto';const sel=makeEl('button','sel',p.selected===false?'○':'✓');sel.type='button';sel.onclick=()=>{selectedPhotos[i].selected=selectedPhotos[i].selected===false?true:false;renderPhotos()};const x=makeEl('button','x','×');x.type='button';x.onclick=()=>{selectedPhotos.splice(i,1);renderPhotos()};d.append(img,sel,x);g.appendChild(d)});if(!selectedPhotos.length){const empty=makeEl('div','tiny','Asnjë foto. Fotot e selektuara me ✓ futen në PDF Employer.');empty.style.gridColumn='1/-1';g.appendChild(empty)}}
...
async function deleteDay(){if(!selectedDay||!data[selectedDay])return;const label=`${String(selectedDay).padStart(2,'0')}.${String(M+1).padStart(2,'0')}.${Y}`;if(!confirm(`A je i sigurt që do ta fshish regjistrimin e datës ${label}?`))return;delete data[selectedDay];await save();closeSheet();render();toast('U fshi')}
async function exportJSON(){const backup=await collectBackup();localStorage.setItem('wr-backup-at',new Date().toISOString());download('WorkLog-Backup-'+Y+'-'+String(M+1).padStart(2,'0')+'.json',JSON.stringify(backup,null,2),'application/json');renderStorageStatus()}
document.getElementById('importInput').onchange=async e=>{const f=e.target.files?.[0];if(!f)return;try{await applyBackup(JSON.parse(await f.text()));localStorage.setItem('wr-backup-at',new Date().toISOString());toast('Backup u importua')}catch{toast('Import i pasuksesshëm')}e.target.value=''};
...
function renderGPS(){const el=document.getElementById('gpsStatus');if(!el)return;el.textContent='';if(!selectedGPS){el.textContent='GPS nuk është marrë ende.';return}const acc=selectedGPS.accuracy?` ±${Math.round(selectedGPS.accuracy)}m`:'';el.appendChild(makeEl('b','','GPS aktiv'+acc));el.appendChild(document.createElement('br'));el.append(selectedGPS.address||'Adresë jo e gjetur');el.appendChild(document.createElement('br'));el.append(`${Number(selectedGPS.lat).toFixed(6)}, ${Number(selectedGPS.lon).toFixed(6)} · ${new Date(selectedGPS.capturedAt).toLocaleString('sq-CH')}`)}
...
async function storageSnapshot(){
  const months={};
  let storageMode=localStorage.getItem('wr-storage-mode')||'IndexedDB';
...
function formatDateTime(v){if(!v)return'Asnjë';try{return new Date(v).toLocaleString('sq-CH')}catch{return v}}
async function renderStorageStatus(){
  const box=document.getElementById('storageRows');if(!box)return;
  const s=await storageSnapshot();box.textContent='';
  [['Ruajtja',s.storageMode],['Muaj',s.monthCount],['Regjistrime',s.entryCount],['Foto',s.photoCount],['Madhësia',humanBytes(s.bytes)],['Backup i fundit',formatDateTime(s.backupAt)]].forEach(([label,value])=>{
    const row=makeEl('div','row');row.appendChild(makeEl('span','',label));row.appendChild(makeEl('b','',String(value)));box.appendChild(row);
  });
}
function renderTodayNotice(){
  const box=document.getElementById('todayNotice');const text=document.getElementById('todayNoticeText');if(!box||!text)return;
  const isCurrent=Y===now.getFullYear()&&M===now.getMonth();const today=now.getDate();
  const show=isCurrent&&!data[today];box.classList.toggle('on',show);
  if(show)text.textContent='Dita e sotme nuk është regjistruar ende.';
}
